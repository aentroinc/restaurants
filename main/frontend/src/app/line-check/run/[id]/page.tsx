...
export default function LineCheckRunPage() {
  ...
  useEffect(() => {
    lineCheckApi.listTemplates()
      .then((tpls) => {
        ...
        if (!found) throw new Error("テンプレートが見つかりません")
        setTpl(found)
      })
  ...
  if (error || !tpl) return <Shell><ErrorState message={error || "テンプレート未取得"} /></Shell>
...
function ItemCard({
  ...
  const tempStatus = useMemo(() => tempCheck(item, draft.valueNumber), [item, draft.valueNumber])
  return (
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <span className="text-[14px]">{current ? "撮り直す" : "カメラで撮影"}</span>
      </button>
      <input
        ref={ref}
        type="file"
...
function tempCheck(item: ChecklistItem, v: number | undefined) {
  if (!item.requires_temperature || v === undefined || v === null || Number.isNaN(v)) return null
  if (item.min_temp != null && v < item.min_temp) return { ok: false, message: `下限割れ (基準 ${item.min_temp}℃)` }
  if (item.max_temp != null && v > item.max_temp) return { ok: false, message: `上限超え (基準 ${item.max_temp}℃)` }
  return { ok: true, message: "範囲内 OK" }
}
  ...
  return (
    <div className="min-h-full bg-[#0a0e14] text-white/80 flex flex-col">
      <header className="flex items-center gap-3 px-4 h-12 border-b border-white/[0.06] bg-[#0c1017]">
        ...
        <h1 className="text-[14px] font-semibold text-white/90">現場チェック実行</h1>
      </header>
      {children}
    </div>
  )
