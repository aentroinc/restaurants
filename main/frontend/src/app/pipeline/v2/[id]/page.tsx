...
import { ReactflowPipelineCanvas } from "@/components/pipeline/ReactflowPipelineCanvas"
...
import {
  Database, Code2, ShieldCheck, Send, Sparkles, Save, Play, Calendar, GitBranch,
  Loader2, History, ChevronLeft, Sparkle,
} from "lucide-react"
...
export default function PipelineEditorV2Page() {
  ...
  return (
    <div className="flex flex-col h-screen">
      <ContextHeader
        title={pipeline.name}
        description={pipeline.description || "DAG パイプラインエディタ (reactflow)"}
        region="-"
        actions={
          <div className="flex items-center gap-2">
            {statusMsg && <span className="text-[11px] text-blue-300 mr-1">{statusMsg}</span>}
            <span className="px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300 text-[10px] inline-flex items-center gap-1 border border-purple-400/30">
              <Sparkle className="w-3 h-3" /> v2 (reactflow)
            </span>
            <Link
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-56 border-r border-white/[0.06] bg-[#0c1017] overflow-y-auto shrink-0">
          ...
          <div className="px-4 py-3 border-t border-white/[0.06]">
            <div className="text-[10px] uppercase tracking-wider text-white/45 font-bold mb-2">操作 (reactflow)</div>
            <ul className="text-[10px] text-white/55 space-y-1">
              <li>• ノードをドラッグで配置</li>
              <li>• ハンドルをドラッグで接続</li>
              <li>• Backspace でノード/エッジ削除</li>
              <li>• ホイールでズーム / 右下で fitview</li>
            </ul>
          </div>
        </aside>
